...
var Node = xbee.Node;
var ZDO = zdo.ZDO;
var Application = zdevice.Application;
...
/**
 * Add an application object to this node or update an existing application object.
...
ZigbeeNode.prototype._putDevice = function(desc) {
	...
	if (typeof application == 'undefined') {
		...
		self.emit("deviceFound", application);
	}
	else {		// update existing application object
		application.update(desc);
		self.emit("deviceUpdated", application);
	}
}
...
ZigbeeNode.prototype.toDesc = function() {
	...
	for (var i=0; i<this.applications.length; i++) {
		apps.push(applications[i].toDesc());
	}
	var desc =   {
