...
// 			
		...
		var handleEndpointDescriptor = function(err, desc) {
			if (err) {
				console.log("error getting endpoint desc: " + util.inspect(err));
			}
			else {
				console.log("persisting endpoint desc: " + util.inspect(desc));
				var endpoint = node.getEndpoint(desc.endpoint);
				self.db.persistEndpoint(endpoint);
...
//	});
	...
	this.zbee.on("initialized", function() {
		self.db.getEndpoints(function(err, endpointDocs) {
			for (var i=0; i<endpointDocs.length; i++) {
				var desc = endpointDocs[i];
				console.log("got stored endpoint: " +desc.address);
			}
		});
	});
