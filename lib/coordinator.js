...
Coordinator.prototype.identifyNode = function(address64, endpoint, cb) {
	var self = this;
	this.db.getNodeByAddress64(address64, function(err, nodeDoc) {
		if (err) {
			...
			if (!node) {
				...
				node.zcl.identify(endpoint, 10, function(err, result) {
	...
	this.db.getNodeByAddress64(address64, function(err, nodeDoc) {
		if (err) {
			...
			if (!node) {
				...
				node.zcl.configureReporting(data.endpoint, data.clusterId, data.configs, function(err, result) {
...
Coordinator.prototype.init = function() {
	...
	this.zbee.on("newNodeDiscovered", function(node) {
		...
		node.on("attributeReport", function(report) {
			handleAttributeReport(null, report);
		});
		...
		var handleApplicationDescriptor = function(err, desc) {
			if (err) {
				...
				self.db.getNodeByAddress16(desc.address16, function(err, node) {
					if (err) {
					}
					else {
					...
					self.db.persistApplication(desc, function(err, app) {
						if (err) {
							console.log("error persisting application: " + err);
						}
						else {
		...
		var handleAttributeReport = function(err, report) {
			console.log("attribute reports: " + JSON.stringify(report));
			self.emit("attributeReport", report);
		}
