	...
	this.db.getNodeByAddress64(address64, function(err, nodeDoc) {
		if (err) {
			...
			if (!node) {
				...
				var seconds = 10;
				console.log("identify not currently available");
				//node.applications[endpoint].identify(seconds);
				//node.applications[endpoint].inputClusters[CLUSTER_IDENTIFY].identify(seconds);
 ...
 */
Coordinator.prototype.getDevices = function(cb) {
...
Coordinator.prototype._handleNode = function(node) {
	...
	node.zdo.on("device", function(desc) {
		self._handleApplicationDescriptor(null, desc);
	});
	...
	if (err) {
		...
		for (var i=0; i<endpoints.length; i++) {
			node.zdo.requestSimpleDescriptor(endpoints[i], function(err, desc) {
				self._handleApplicationDescriptor(err, desc);
			});
		}
	}
}

Coordinator.prototype._handleApplicationDescriptor = function(err, desc) {
	if (err) {
