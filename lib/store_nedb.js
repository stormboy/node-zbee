...
DataStore.prototype._init = function() {
	...
	var self = this;
	this.db.nodes.loadDatabase(function (err) {    // Callback is optional
		self.db.nodes.ensureIndex({ fieldName: 'address64', unique: true });
		self.db.nodes.ensureIndex({ fieldName: 'address16', unique: false });
	});
	this.db.endpoints.loadDatabase(function (err) {    // Callback is optional
		//self.db.endpoints.ensureIndex({ fieldName: 'address', unique: true }, function (err) {});
	});
	this.db.bindings.loadDatabase(function (err) {    // Callback is optional
		//self.db.bindings.ensureIndex({ fieldName: 'address', unique: true }, function (err) {});
	});
