...
ZBee.prototype.createSourceRoute = function(addresses, remote64, remote16,
		_cb) {
	var packets = [];
 ...
 */
var ZigbeeNode = function(zbee, params, data_parser) {
...
ZigbeeNode.prototype.requestNodeDescriptor = function(cb) {
	this.zdo.requestNodeDescriptor(cb);
}
ZigbeeNode.prototype.requestActiveEndpoints = function(cb) {
	this.zdo.requestActiveEndpoints(cb);
}
ZigbeeNode.prototype.requestSimpleDescriptor = function(endpoint, cb) {
	this.zdo.requestSimpleDescriptor(endpoint, cb);
}
...
ZigbeeNode.prototype._onExplicitRx = function(packet) {
	console.log("got RX: " + util.inspect(packet));
