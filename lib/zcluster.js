 ...
 * A cluster.
 */
var Cluster = exports.Cluster = function(device, clusterId, direction) {
	EventEmitter.call(this);
	
	this.device  = device;
	this.node = device.node;
	this.zbee = device.zbee;
 ...
 * 
 * device: Device (Application object)
 * clusterId: int(2)
 * direction: 0=in, 1=out
 * 
 */
Cluster.create =  function(device, clusterId, direction) {
	//console.log("creating cluster for device: " + device + " : " + clusterId + " : " + direction);
	
	// create appropriate cluster based on clusterId
	var profile = Profiles[device.profileId];
	if (profile && profile.Clusters) {
		var ClusterType = profile.Clusters[clusterId];
		if (ClusterType) {
			return new ClusterType(device, direction);
		}
	}
	return new Cluster(device, clusterId, direction);
}
 ...
 * config example
 * {
 * 	 direction: 0x01,			// 0 = send report, 1 = expect to receive report
 *   id: 0x0001,				// attribute id
 *   type: 0x01,                         // if direction == 0. data type
 *   minInterval: 0x0001,                // if direction == 0. min interval in seconds to report
 *   maxInterval: 0x0001,                // if direction == 0. max interval in seconds. if 0xFFFF, then cancel/no report
...
Cluster.prototype.sendOnOff = function(value, cb) {
	...
	var data = new Buffer([ frameControl, this.zbee._getTxnId(), command, 0x00, 0x00 ]);
	this.node.sendZclCommand(clusterId, profileId, sourceEndpoint, this.endPoint, data, cb);
	*/
	
	var command = value ? 0x01 : 0x00;
	this._sendClusterCommand(command, null, cb);
}
