...
var ZCL = exports.ZCL = function(zbee, node) {
	this.zbee = zbee;
	this.node = node;
}
 ...
 */
ZCL.prototype.readAttributes = function(endpoint, attributes, cb) {

	var clusterId = 0x0000		// Basic cluster
	var profileId = 0x0104;		// HA : TODO use profileID supported on the device
	var sourceEndpoint = endpoint;

	var frameControl = 0x00;	// Command acts across the entire profile
	var txn = this.zbee._getTxnId();
	var command = GeneralCommands.READ_ATTRIBUTES;		// Read Attributes command
	
	var payload = [ frameControl, txn, command ];
	if (attributes) {
		for (var i=0; i<attributes; i++) {
			attrId = attributes[i];
			payload.push(attrId & 0xFF);
			payload.push((attrId >> 8) & 0xFF);
		}
	}

	this.node.sendZclCommand(clusterId, profileId, sourceEndpoint, endpoint, new Buffer(payload), cb);
}
 ...
 */
ZCL.prototype.writeAttributes = function(endpoint, records, cb) {
	...
	var frameControl = 0x00;	// Command acts across the entire profile
	var txn = this.zbee._getTxnId();
	var command = GeneralCommands.WRITE_ATTRIBUTES;		// Write Attributes command
	
	var payload = [ frameControl, txn, command ];
	if (records) {
		for (var i=0; i<records; i++) {
			record = records[i];
			payload.push(record.id & 0xFF);
			payload.push((record.id >> 8) & 0xFF);
			payload.push(record.type);
 ...
 */
// 8, 5, 7, 141, 0, 0, 0 
ZCL.prototype.configureReporting = function(endpoint, clusterId, configs, cb) {
	...
	var frameControl = 0x00;	// Command acts across the entire profile
	var txn = this.zbee._getTxnId();
	var command = GeneralCommands.CONFIGURE_REPORTING;		// Report Configuration command
	
	var payload = [ frameControl, txn, command ];
	if (configs) {
		for (var i=0; i<configs.length; i++) {
			config = configs[i];
			payload.push(config.direction);
			if (config.direction == 0) {
				payload.push(config.id & 0xFF);
				payload.push((config.id >> 8) & 0xFF);
				payload.push(config.type);
				var minInterval = config.minInterval ? config.minInterval : 0x0001;
				payload.push(minInterval & 0xFF); payload.push((minInterval >> 8) & 0xFF);
				var maxInterval = config.maxInterval ? config.maxInterval : 360;
				payload.push(maxInterval & 0xFF);
				payload.push((maxInterval >> 8) & 0xFF);
				var change = config.change;
				if (change) {
					for (var j=0; j<change.length; j++) {
						payload.push(change[j]);
					}
				}
			}
			else {
				payload.push(config.timeout & 0xFF);
				payload.push((config.timeout >> 8) & 0xFF);
			}
		}
	}
	console.log("config reporting payload: " + util.inspect(payload));

	this.node.sendZclCommand(clusterId, profileId, sourceEndpoint, endpoint, new Buffer(payload), cb);
}


ZCL.prototype.discoverAttributes = function(endpoint, clusterId, start, max, cb) {
	...
	var frameControl = 0x00;	// Command acts across the entire profile
	var txn = this.zbee._getTxnId();
	var command = GeneralCommands.DISCOVER_ATTRIBUTES;		// Report Configuration command
	
	var payload = [ frameControl, txn, command ];
	payload.push(start & 0xFF);
	payload.push((start >> 8) & 0xFF);
	payload.push(max);
	
	console.log("discover attributes: " + util.inspect(payload));

	this.node.sendZclCommand(clusterId, profileId, sourceEndpoint, endpoint, new Buffer(payload), cb);
}

/**
 * Handle explicit message addressed from node
 */
ZCL.prototype.handleRx = function(packet) {
 ...
 */
ZCL.prototype._handleClusterCommand = function(packet) {
 ...
 */
ZCL.prototype._handleGeneralCommand = function(packet) {
	var data = packet.rawData;
	var command = data[2];
...
ZCL.prototype._handleReadAttributesResponse = function(packet) {
	var data = packet.rawData;
	...
	this.node.emit("attributeReport", report);
}

ZCL.prototype._handleWriteAttributes = function(packet) {
	var data = packet.rawData;
 ...
 */
ZCL.prototype._handleReportedAttributes = function(packet) {
...
ZCL.prototype.identify = function(endpoint, seconds, cb) {
	//console.log("identifying");
	var frameControl = 0x01;	// command is specific to cluster
	var clusterId = 0x0003		// Identify
	var profileId = 0x0104;		// HA
